...
namespace ns.Character
{
    ...
    public class CharacterAnimationHandler : MonoBehaviour
    {
        ...
        public virtual void PlayTargetAnimation(string targetAnima, bool isInteracting, float normalizedTransitionDuration, int layer = -1)
        {
            Debug.Log($"{Time.frameCount}帧播放动画：{targetAnima}，是否交互：{isInteracting}");//本质是用于标记动画是否完结
            anim.SetBool("IsInteracting", isInteracting);
            anim.CrossFade(targetAnima, normalizedTransitionDuration, layer);
