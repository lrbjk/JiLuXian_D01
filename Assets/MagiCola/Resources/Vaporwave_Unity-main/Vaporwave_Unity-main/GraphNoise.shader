    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float4 frag(v2f i) : SV_Target
            {
                ...
                float light = (ScaledGaussianRandom(float3(i.uv * 10, _Time.x), 0.5, 0.4) * _LightNoise - _LightNoise /
                    2);
                color = color + float3(light, light, light);
                float dark = (ScaledGaussianRandom(float3(i.uv * 5, _Time.x * 2), 0.5, 0.4) * _DrakNoise - _DrakNoise /
